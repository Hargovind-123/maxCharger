...
module.exports = {
    signup: async (req, res) => {
        try {
            const { owner,vehicle,vehicleNumber,vehicleType, regDate,mfgDate,  email,regNumber, password, confirm_password, mobileNumber} = req.body
                    ...
                    try {
                     ...
                     console.log("===>39", hashPassword)
                      ...
                      const doc = await userModel({ email:email})
                        ...
                        return res.send({ responseCode: 200, responseMesage: "User is register Successfully", responsResult: userSave});
                    }
                    catch (error) {
                        console.log(error)
                    }
 ...
 verifyOtp:
    async (req, res) => {
        try {
            ...
            if (!user) {
                res.send({ responseCode: 409, responseMesage: "user  not  exist" })
...
changePassword: async (req, res) => {
    ...
    editProfile: async (req,res)=>{
        try {
           const{owner,vehicle, vehicleType,regDate,email, regNumber, mfgDate,}=req.body
        ...
        if(mail==user.email){
            ...
            if(number==user.number){
               ...
               let userProfileUpdate=await userModel.findByIdAndUpdate({_id:user._id},{email:email,owner:owner,vehicle:vehicle,vehicleType:vehicleType,vehicleNumber:vehicleNumber,regDate:regDate,mfgDate:mfgDate,mobileNumber:mobileNumber},{new:true})
